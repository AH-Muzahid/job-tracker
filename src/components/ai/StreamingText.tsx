...
function sourceImage(source: SourceItem) {
...
const ACTION_ICONS: { key: string; label: string; icon: React.ReactNode }[] = [
  {
    key: "copy",
    label: "Copy message",
    icon: (
...
export default function StreamingText({
  ...
  let effectiveTokens = customTokens;
  if (!effectiveTokens && rawText) {
    effectiveTokens = rawText.split(" ").map((t) => ({ text: t }));
  }
  if (!effectiveTokens) {
    effectiveTokens = DEFAULT_TOKENS;
  }

  const [count, setCount] = useState(0);
  const [sourcesOpen, setSourcesOpen] = useState(false);
  const done = count >= effectiveTokens.length;
  ...
  useEffect(() => {
    ...
    const t = setTimeout(
      () => setCount((c) => (c >= effectiveTokens.length ? 0 : c + 1)),
      done ? HOLD_MS : WORD_MS,
    );
    return () => clearTimeout(t);
  }, [count, done, loop, effectiveTokens.length, onDone]);

  const handleAction = (key: string) => {
    if (key === "copy") {
      const fullText = effectiveTokens.map((t) => t.text).filter(Boolean).join(" ");
  ...
  return (
    <div className={`select-none ${fill ? "w-full" : "min-h-[15.5rem] w-full max-w-95"} ${className}`}>
      <p className="text-[13px] leading-relaxed text-ink">
        {effectiveTokens.slice(0, count).map((token, i) =>
          token.cite ? (
            <SourceChip key={i} source={sources[token.sourceIndex ?? 0] || sources[0]} />
          ) : (
