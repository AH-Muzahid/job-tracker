...
import { ExternalJobOpportunity, UnifiedRawJob } from "./types"
import {
  ...
  try {
    ...
    if (resume?.textContent) {
      ...
      canonicalTokens.forEach((tok: string) => {
        if (Object.prototype.hasOwnProperty.call(CANONICAL_ALIASES, tok)) {
    ...
    const userWorkPreference = (profile?.workPreference || "open").toLowerCase()
    ...
    for (const job of rawJobs) {
      ...
      if (!isExplicitSearch) {
        ...
        // Gate 1D: International candidates will NEVER see foreign-only restricted remote jobs
        ...
        // Gate 1E: Candidate targeting tech/software roles will NEVER see non-tech positions
      ...
      // =========================================================================
      // STAGE 2: MULTI-FACTOR FIT SCORING & IN-DEPTH PERSONALIZATION (Scale: 1% - 99%)
      // =========================================================================
      ...
      // Factor 8: Semantic Vector Matching (REC-17)
      const semanticResult = computeSemanticSimilarity(
        {
          targetRoles: targetRolesLower,
          skills: Array.from(userSkills),
          projectSummaries: bestProjects.map((p: any) => `${p.name || ""} ${p.stack || ""} ${p.description || ""}`),
        },
        {
          title: position,
          description,
      ...
      if (jdTechSkills.size > 0) {
        ...
        atsScore = Math.round(48 + matchRatio * 44)
      ...
      if (bestProjects.length > 0) {
        const matchingProject = bestProjects.find((p: any) => {
          if (!p.stack) return false
          const stackTokens = p.stack.toLowerCase().split(/[,/|\s-]+/)
          return stackTokens.some((tok: string) => tok.length > 2 && fullJobText.includes(tok))
        })

        if (matchingProject?.name) {
          projectRecommendation = `Highlight your '${matchingProject.name}' project to demonstrate verified hands-on ${matchingProject.stack?.split(",").slice(0, 3).join(", ")} execution.`
        } else {
          const firstProject = bestProjects[0]
          if (firstProject?.name) {
            projectRecommendation = `Showcase your '${firstProject.name}' project in your application to validate hands-on engineering mastery.`
          }
        }
      }
      ...
      if (job.authorName || job.sourceBoard === "linkedin_post" || job.sourceBoard === "company_portal") {
        ...
        const topProjName = topProject?.name || "portfolio projects"
      ...
      if (semanticResult.rationale) {
        rationaleParts.push(`🧠 ${semanticResult.rationale}`)
      }
      ...
      scoredOpportunities.push({
        ...
        companyEnrichment: getCompanyEnrichment(company, {
          description,
          location: jobLocation,
          url: job.url,
          tags,
